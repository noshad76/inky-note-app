// features/note/components/DeleteConfirmDialog.tsx
"use client";
  ...
  return (
    <Dialog.Root open={isOpen} onOpenChange={(d) => !d.open && onClose()}>
      ...
      <Dialog.Positioner className="fixed inset-0 z-50 flex items-center justify-center p-4">
        <Dialog.Content className="w-full max-w-sm rounded-xl bg-surface p-6 shadow-xl border border-border">
          <div className="flex items-center gap-3 text-danger mb-4">
            <div className="p-2 bg-danger/10 rounded-full"><AlertCircle size={20} /></div>
            <Dialog.Title className="text-base font-bold text-text">{t("deleteTitle")}</Dialog.Title>
          </div>
